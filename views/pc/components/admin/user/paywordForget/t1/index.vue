  ...
  <Form :model="formData" status-icon ref="pwdForm" :label-width="110" class="password-ruleForm">
    ...
    <FormItem label="旧密码" prop="password">
      <Input :type="passwordType.password" v-model="formData.password" placeholder="请输入旧的登陆密码">
        <Button slot="append" @click="showPwd('password')" v-if="passwordType.password=='password'"><Icon type="ios-eye" /></Button>
        <Button slot="append" @click="showPwd('password')" v-else><Icon type="ios-eye-off" /></Button>
      </Input>
    </FormItem>
    ...
    <FormItem label="新密码" prop="new_password">
      <Input :type="passwordType.new_password" v-model="formData.new_password" auto-complete="off" placeholder="请输入新的登陆密码">
        <Button slot="append" @click="showPwd('new_password')" v-if="passwordType.new_password=='password'"><Icon type="ios-eye" /></Button>
        <Button slot="append" @click="showPwd('new_password')" v-else><Icon type="ios-eye-off" /></Button>
      </Input>
    </FormItem>
    ...
    <FormItem label="确认密码" prop="confirm_password">
      <Input :type="passwordType.confirm_password" v-model="formData.confirm_password" auto-complete="off" placeholder="请再次输入新的登陆密码">
        <Button slot="append" @click="showPwd('confirm_password')" v-if="passwordType.confirm_password=='password'"><Icon type="ios-eye" /></Button>
        <Button slot="append" @click="showPwd('confirm_password')" v-else><Icon type="ios-eye-off" /></Button>
      </Input>
    </FormItem>
