<template>
  <div class="drawmoney">
    ...
    <div class="withdraw-p" v-if="isAgent">
      <p>
        老虎机佣金余额：
        <span style="color:red;">{{wallet['SLOT']}}</span>元
      </p>
      <p>
        其他佣金余额：
        <span style="color:red;">{{wallet['LIVE']}}</span>元
      </p>
    </div>
    <div class="withdraw-p" v-if="isUser">
      <p>
        主账户余额：
        <span style="color:red;">{{wallet['MAIN']}}</span>元
      </p>
    ...
    >
      ...
      <FormItem label="选择取款类型：" prop="tkType" v-if="isAgent">
        <Select v-model="sumitData.tkType" placeholder="请选择取款类型">
          <Option v-for="item in drawType" :key="item.value" :label="item.name" :value="item.value"></Option>
        </Select>
      </FormItem>
      ...
      <FormItem label="选择取款银行：" prop="id">
        ...
        <nuxt-link :to="{name:'user_banks'}" class="addbanks" v-if="showAdd">
    ...
    <Dialog title="取款成功" width="640px" :visible.sync="drawSuccess">
      <Alert
        title="提交提款申请"
        :description="`工作人员正在为您处理，如果您的各项资料正确，取款金额会在5-10分钟内到达您的绑定银行账户`"
        :closable="false"
        type="warning"
      ></Alert>
      <br>
...
import { withdraw } from "@@/mixins/money/withdraw";
